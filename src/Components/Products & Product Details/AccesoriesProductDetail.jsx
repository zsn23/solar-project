...
const AccesoriesProductDetail = () => {
    const { id } = useParams();
    const product = productsAccesories.find((p) => p.id === parseInt(id));
    ...
    if (!product) {
        return <div className="container-fluid mt-5 mb-5 text-center fs-1"><div className="row g-0"><div className="col-12"><h1>Product not found</h1></div></div></div>;
    }
            ...
            <div className="container-fluid ">
                ...
                <div className="row g-0 justify-content-center">
                    {productsAccesories.map((productsAccesories) => (
                        <div key={productsAccesories.id} className="col-lg-3 col-md-6 col-sm-6 col-12 mb-4 p-0 d-flex justify-content-center align-items-center">
                            <Link to={`/AccesoryProduct/${productsAccesories.id}`} className="Customcard d-flex flex-column text-decoration-none text-dark">
                                <div className="Customcard-img d-flex justify-content-center">
                                    <img src={productsAccesories.image[0]} alt={productsAccesories.title} className="Customcard-img-top" />
                                </div>
                                <div className="d-flex justify-content-between flex-column h-100">
                                    <div className="Customcard-info d-flex flex-column justify-content-center align-items-center">
                                        <span className="Customtext-title d-flex flex-nowrap">{productsAccesories.title} </span>
                                        <span className="Customtext-body">{productsAccesories.descrition}</span>
                                    </div>
                                    <div className="Customcard-footer border-top border-dark">
                                        <span className="Customtext-title">{productsAccesories.price}</span>
...
export default AccesoriesProductDetail;